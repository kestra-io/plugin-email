...
import io.kestra.plugin.email.MailReceivedTrigger;
import io.kestra.plugin.email.MailService;
...
class MailReceivedTriggerTest extends AbstractTriggerTest {
    ...
    void MailReceivedTriggerWithPop3() throws Exception {
        ...
        executionQueue.addListener(execution -> {
            if (execution.getFlowId().equals("mail-received-trigger-pop3")) {
    ...
    void MailReceivedTriggerWithImap() throws Exception {
        ...
        executionQueue.addListener(execution -> {
            if (execution.getFlowId().equals("mail-received-trigger-imap")) {
                lastExecution.set(execution);
                queueCount.countDown();
            }
        });
